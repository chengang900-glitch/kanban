...
(deftest generated-question-language-test
  (testing "table and metric templates require Simplified Chinese questions"
    (doseq [path ["metabot/prompts/example_questions_table.selmer"
                  "metabot/prompts/example_questions_metric.selmer"]]
      (let [template (#'native-generator/load-template path)]
        (is (str/includes? template "Write every generated question in natural Simplified Chinese."))
        (is (str/includes? template "Do not output an English question.")))))
  (testing "structured output schema also requires Simplified Chinese questions"
    (is (str/includes? (get-in @#'native-generator/questions-json-schema
                               [:properties :questions :description])
                       "Simplified Chinese"))))
