...
(deftest ^:parallel message->chat-messages-reasoning-test
  (testing "legacy assistant reasoning blocks remain readable for backward-compatible history normalization"
    (is (= [{:id "reasoning-1"
             :role "agent"
             :type "reasoning"
           ...
           (metabot-persistence/message->chat-messages
            {:role :assistant
             :data [{:type "reasoning" :id "reasoning-1" :text "Checking the data"}]})))))
...
(deftest ^:parallel messages->history-test
  (testing "reconstructs resumable user, assistant, and tool history"
    (let [history (metabot-persistence/messages->history
                   [{:role :user
                     :data [{:role "user" :content "find orders"}]}
                    {:role :assistant
                     :finished true
                     :data [{:type "text" :text "I will look."}
                            {:type "tool-input" :id "call-1" :function "search"
                             :arguments {:q "orders"}}
                            {:type "tool-output" :id "call-1"
                             :result {:output {:rows 3}}}]}])]
      (is (= [{:role "user" :content "find orders"}
              {:role "assistant" :content "I will look."}
              {:role "assistant"
               :tool_calls [{:id "call-1" :name "search" :arguments "{\"q\":\"orders\"}"}]}
              {:role "tool" :tool_call_id "call-1" :content "{\"rows\":3}"}]
             history))
      (is (mr/validate ::metabot.schema/messages
                       (mc/decode ::metabot.schema/messages history (mtx/string-transformer)))
          "restored history must satisfy the agent-streaming request schema")))
  (testing "serializes a persisted result map when no :output field exists"
    (let [[tool-result] (metabot-persistence/messages->history
                         [{:role :assistant
                           :finished true
                           :data [{:type "tool-output" :id "call-2"
                                   :result {:ok true}}]}])]
      (is (= "{\"ok\":true}" (:content tool-result)))
      (is (string? (:content tool-result)))))
  (testing "adds a synthetic result for an interrupted tool call"
    (is (= {:role "tool"
            :tool_call_id "call-2"
            :content "Tool execution interrupted by user"}
           (last
            (metabot-persistence/messages->history
             [{:role :assistant
               :finished false
               :data [{:type "tool-input" :id "call-2" :function "search"
                       :arguments {}}]}]))))))
    ...
    (t2/with-transaction [_conn nil {:rollback-only true}]
      (let [conversation-id (str (random-uuid))
        ...
        (let [detail (metabot-persistence/conversation-detail conversation-id)
          ...
          (is (= [{:role "assistant" :content "first"}
