...
(deftest request-applies-configured-llm-timeouts-test
  (with-redefs [http/request identity
                llm.settings/llm-request-timeout-ms (constantly 120000)
                llm.settings/llm-connection-timeout-ms (constantly 7000)]
    (is (=? {:url "https://api.example.com/chat/completions"
             :socket-timeout 120000
             :connection-timeout 7000}
            (self.core/request {:url "https://api.example.com"
...
(deftest ^:parallel parse-provider-model-test
  (testing "parses provider/model format correctly"
    ...
    (is (=? {:provider "openai-compatible" :model "deepseek-v4-pro" :ai-proxy? false}
            (#'self/parse-provider-model "openai-compatible/deepseek-v4-pro"))))
...
(deftest ^:parallel resolve-adapter-test
  (testing "resolves known providers to adapter functions"
    ...
    (is (fn? (#'self/resolve-adapter "openai-compatible")))
...
(deftest aisdk-line-xf-test
  (testing "converts internal parts to AI SDK v4 line protocol, skipping usage by default"
    (let [parts [{:type :start :id "msg-1"}
                 {:type :reasoning :text "Checking the data"}
      ...
      ;; Raw reasoning and usage are both skipped from the user-facing protocol.
      (is (= 5 (count lines)))
