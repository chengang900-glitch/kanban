...
(deftest combine-text-parts-xf-test
  ...
  (testing "combines consecutive reasoning parts without merging them into text"
    (is (= [{:type :reasoning, :text "checking data"}
            {:type :text, :text "answer"}]
           (into [] (metabot.persistence/combine-text-parts-xf)
                 [{:type :reasoning, :text "checking "}
                  {:type :reasoning, :text "data"}
                  {:type :text, :text "answer"}]))))
...
(deftest settings-validate-openai-compatible-does-not-save-test
  (mt/with-temp-env-var-value! [mb-llm-openai-compatible-api-key nil
                                mb-llm-openai-compatible-api-base-url nil
                                mb-llm-metabot-provider nil]
    (mt/with-temporary-setting-values [metabot.settings/llm-metabot-provider "anthropic/claude-haiku-4-5"
                                       llm.settings/llm-openai-compatible-api-key nil
                                       llm.settings/llm-openai-compatible-api-base-url nil]
      (mt/with-dynamic-fn-redefs
        [metabot.self/validate-model
         (fn [provider opts]
           (is (= "openai-compatible" provider))
           (is (= {:model "model-a"
                   :credentials {:api-key "provider-key"
                                 :base-url "https://api.example/v1"}}
                  opts))
           true)
         metabot.self/list-models
         (fn [provider {:keys [credentials]}]
           (is (= "openai-compatible" provider))
           (is (= {:api-key "provider-key"
                   :base-url "https://api.example/v1"}
                  credentials))
           {:models [{:id "model-a" :display_name "Model A"}]})]
        (is (= {:value "anthropic/claude-haiku-4-5"
                :models [{:id "model-a" :display_name "Model A"}]}
               (mt/user-http-request :crowberto :post 200 "metabot/settings/validate"
                                     {:provider "openai-compatible"
                                      :model "model-a"
                                      :credentials {:api-key "provider-key"
                                                    :base-url "https://api.example/v1"}})))
        (is (nil? (llm.settings/llm-openai-compatible-api-key)))
        (is (nil? (llm.settings/llm-openai-compatible-api-base-url)))
        (is (= "anthropic/claude-haiku-4-5"
               (metabot.settings/llm-metabot-provider)))))))

(deftest settings-put-openai-compatible-saves-credentials-and-model-test
  (mt/with-temp-env-var-value! [mb-llm-openai-compatible-api-key nil
                                mb-llm-openai-compatible-api-base-url nil
                                mb-llm-metabot-provider nil]
    (mt/with-temporary-setting-values [metabot.settings/llm-metabot-provider "anthropic/claude-haiku-4-5"
      ...
      (mt/with-dynamic-fn-redefs
        [metabot.self/validate-model
         (fn [provider {:keys [credentials model]}]
           (is (= "openai-compatible" provider))
           (is (= {:api-key "provider-key"
                   :base-url "https://api.example/v1"}
                  credentials))
         ...
         (fn [_provider _opts]
           {:models [{:id "model-a" :display_name "Model A"}]})]
        (is (= {:value "openai-compatible/model-a"
                :models [{:id "model-a" :display_name "Model A"}]}
               (mt/user-http-request :crowberto :put 200 "metabot/settings"
                                     {:provider "openai-compatible"
                                      :model "model-a"
                                      :credentials {:api-key "provider-key"
                                                    :base-url "https://api.example/v1/"}})))
        (is (= "provider-key" (llm.settings/llm-openai-compatible-api-key)))
        (is (= "https://api.example/v1"
               (llm.settings/llm-openai-compatible-api-base-url)))
        (is (= "openai-compatible/model-a"
               (metabot.settings/llm-metabot-provider)))))))

(deftest settings-put-openai-compatible-model-only-change-preserves-credentials-test
  (mt/with-temp-env-var-value! [mb-llm-metabot-provider nil]
    (mt/with-temporary-setting-values [metabot.settings/llm-metabot-provider "openai-compatible/model-a"
                                       llm.settings/llm-openai-compatible-api-key "saved-key"
                                       llm.settings/llm-openai-compatible-api-base-url "https://api.example/v1"]
      (mt/with-dynamic-fn-redefs
        [metabot.self/validate-model
         (fn [_provider {:keys [credentials model]}]
           (is (= {:api-key "saved-key" :base-url "https://api.example/v1"}
                  credentials))
           (is (= "model-b" model))
           true)
         metabot.self/list-models
         (fn [_provider _opts]
           {:models [{:id "model-b" :display_name "Model B"}]})]
        (is (= "openai-compatible/model-b"
               (:value (mt/user-http-request :crowberto :put 200 "metabot/settings"
                                             {:provider "openai-compatible"
                                              :model "model-b"}))))
        (is (= "saved-key" (llm.settings/llm-openai-compatible-api-key)))
        (is (= "https://api.example/v1"
               (llm.settings/llm-openai-compatible-api-base-url)))))))

(deftest settings-put-openai-compatible-protocol-only-change-test
  (mt/with-temp-env-var-value! [mb-llm-openai-compatible-api-protocol nil]
    (mt/with-temporary-setting-values [metabot.settings/llm-metabot-provider "openai-compatible/model-a"
                                       llm.settings/llm-openai-compatible-api-key "saved-key"
                                       llm.settings/llm-openai-compatible-api-base-url "https://api.example/v1"
                                       llm.settings/llm-openai-compatible-api-protocol "chat-completions"]
      (mt/with-dynamic-fn-redefs
        [metabot.self/validate-model
         (fn [_provider {:keys [credentials model]}]
           (is (= {:api-key "saved-key"
                   :base-url "https://api.example/v1"
                   :protocol "responses"}
                  credentials))
           (is (= "model-a" model))
         ...
         (fn [_provider {:keys [credentials]}]
           (is (= "responses" (:protocol credentials)))
           {:models [{:id "model-a" :display_name "Model A"}]})]
        (is (= "openai-compatible/model-a"
               (:value (mt/user-http-request :crowberto :put 200 "metabot/settings"
                                             {:provider "openai-compatible"
                                              :credentials {:protocol "responses"}}))))
        (is (= "responses"
               (llm.settings/llm-openai-compatible-api-protocol)))))))

(deftest settings-put-openai-compatible-disconnect-clears-credentials-test
  (mt/with-temp-env-var-value! [mb-llm-openai-compatible-api-key nil
                                mb-llm-openai-compatible-api-base-url nil
                                mb-llm-openai-compatible-api-protocol nil]
    (mt/with-temporary-setting-values [metabot.settings/llm-metabot-provider "openai-compatible/model-a"
                                       llm.settings/llm-openai-compatible-api-key "saved-key"
                                       llm.settings/llm-openai-compatible-api-base-url "https://api.example/v1"
      ...
      (mt/with-dynamic-fn-redefs
        [metabot.self/list-models
         (fn [_provider _opts]
           (is false "model listing should not run while clearing credentials"))]
        (is (= {:value "openai-compatible/model-a" :models []}
               (mt/user-http-request :crowberto :put 200 "metabot/settings"
                                     {:provider "openai-compatible"
                                      :credentials nil})))
        (is (nil? (llm.settings/llm-openai-compatible-api-key)))
