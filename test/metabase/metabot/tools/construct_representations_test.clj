...
(deftest sqlserver-declare-source-card-is-not-composable-test
  (testing "SQL Server DECLARE batches fail before a chart can be reported as successful"
    (with-redefs [tools.u/get-card-by-entity-id
                  (fn [_]
                    {:database_id 42
                     :dataset_query {:type :native
                                     :native {:query " -- setup\n/* vars */ DECLARE @d date; SELECT @d"}}})
                  t2/select-one-fn (fn [& _] :sqlserver)]
      (try
        (#'construct/assert-composable-source-card!
         {"stages" [{"source-card" "native-model-eid"}]})
        (is false "expected non-composable source error")
        (catch clojure.lang.ExceptionInfo e
          (is (= :non-composable-native-source (:error (ex-data e))))
          (is (true? (:agent-error? (ex-data e))))))))
  (testing "ordinary SELECT-based SQL Server models remain composable"
    (with-redefs [tools.u/get-card-by-entity-id
                  (fn [_]
                    {:database_id 42
                     :dataset_query {:type :native
                                     :native {:query "SELECT * FROM dbo.orders"}}})
                  t2/select-one-fn (fn [& _] :sqlserver)]
      (is (nil? (#'construct/assert-composable-source-card!
                 {"stages" [{"source-card" "native-model-eid"}]}))))))
