...
(deftest metabot-prompt-source-cards-test
  (testing "prompt sources include questions in scope and cards used by dashboards in scope"
    (mt/dataset test-data
      (mt/with-temp [:model/Collection selected-coll {:name "Selected collection"}
                     :model/Collection other-coll    {:name "Other collection"}
                     :model/Card direct-question     {:type :question :collection_id (:id selected-coll)}
                     :model/Card dashboard-question  {:type :question :collection_id (:id other-coll)}
                     :model/Card unrelated-question  {:type :question :collection_id (:id other-coll)}
                     :model/Dashboard dashboard      {:collection_id (:id selected-coll)}
                     :model/DashboardCard _          {:dashboard_id (:id dashboard)
                                                      :card_id (:id dashboard-question)}
                     :model/Metabot metabot          {:name "Prompt source Metabot"
                                                      :collection_id (:id selected-coll)
                                                      :use_verified_content false}]
        (let [card-ids (mt/with-test-user :crowberto
                         (into #{} (map :id)
                               (metabot.tools.util/get-prompt-source-cards (:id metabot))))]
          (is (contains? card-ids (:id direct-question)))
          (is (contains? card-ids (:id dashboard-question)))
          (is (not (contains? card-ids (:id unrelated-question)))))))))
