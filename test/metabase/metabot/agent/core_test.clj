...
(deftest max-iterations-surfaces-typed-error-test
  (mt/as-admin
    (mt/with-temporary-setting-values [llm-metabot-provider test-provider]
      (let [calls (atom 0)]
        (mt/with-dynamic-fn-redefs
          ...
          (let [parts (into [] (agent/run-agent-loop
                                {:messages   [{:role :user :content "keep searching"}]
                                 ...
                                 :profile-id :internal
                                 :context    {}}))
                error (some #(when (= :error (:type %)) (:error %)) parts)]
            (is (= 20 @calls))
            (is (= :metabot_max_iterations (:error-code error)))
            (is (re-find #"step limit" (:message error)))))))))
