...
(deftest list-conversations-filters-by-profile-test
  (testing "GET /api/metabot/conversations filters conversations by profile-id"
    (let [user-id (mt/user->id :rasta)]
      (mt/with-temp [:model/MetabotConversation {nlq-convo :id} {:user_id user-id :summary "NLQ"}
                     :model/MetabotConversation {internal-convo :id} {:user_id user-id :summary "Internal"}
                     :model/MetabotMessage _ {:conversation_id nlq-convo
                                              :user_id         user-id
                                              :profile_id      "nlq"}
                     :model/MetabotMessage _ {:conversation_id internal-convo
                                              :user_id         user-id
                                              :profile_id      "internal"}]
        (let [response (mt/user-http-request :rasta :get 200 "metabot/conversations?profile-id=nlq")
              ids      (set (map :conversation_id (:data response)))]
          (is (= #{nlq-convo} ids))
          (is (= 1 (:total response))))))))
    ...
    (let [user-id (mt/user->id :rasta)]
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id user-id :summary "mine"}
        ...
        (let [response (mt/user-http-request :rasta :get 200
          ...
          (is (= [{:role "user" :content "hello"}] (:history response)))
          (is (= {} (:state response))))))))
  ...
  (testing "GET /api/metabot/conversations/:id returns 403 for a non-participant non-admin"
    (let [owner-id (mt/user->id :rasta)]
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id owner-id}
        ...
        (is (some? (mt/user-http-request :lucky :get 403
                                         (str "metabot/conversations/" convo-id))))))))
...
(deftest get-conversation-404-test
  (testing "GET /api/metabot/conversations/:id returns 404 when the conversation does not exist"
    (is (some? (mt/user-http-request :rasta :get 404
                                     (str "metabot/conversations/" (random-uuid)))))))

(deftest get-conversation-validates-profile-test
  (testing "GET /api/metabot/conversations/:id validates the requested profile-id"
    (let [user-id (mt/user->id :rasta)]
        ...
        (is (= convo-id
               (:conversation_id
                (mt/user-http-request :rasta :get 200
                                      (str "metabot/conversations/" convo-id "?profile-id=nlq")))))
        (is (some? (mt/user-http-request :rasta :get 404
                                         (str "metabot/conversations/" convo-id "?profile-id=internal"))))))))

(deftest delete-conversation-authentication-test
  (testing "DELETE /api/metabot/conversations/:id requires auth"
    (is (= "Unauthenticated"
           (mt/client :delete 401 (str "metabot/conversations/" (random-uuid)))))))

(deftest delete-conversation-originator-test
  (testing "the originator can permanently delete a conversation and its messages"
    (let [user-id (mt/user->id :rasta)]
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id user-id}
                     :model/MetabotMessage _ {:conversation_id convo-id
                                              :user_id         user-id
                                              :profile_id      "internal"}]
        (is (nil? (mt/user-http-request :rasta :delete 204
                                        (str "metabot/conversations/" convo-id
                                             "?profile-id=internal"))))
        (is (not (t2/exists? :model/MetabotConversation :id convo-id)))
        (is (not (t2/exists? :model/MetabotMessage :conversation_id convo-id)))))))

(deftest delete-conversation-non-originator-forbidden-test
  (testing "a participant cannot delete a conversation they did not originate"
    (let [originator-id (mt/user->id :rasta)
          participant-id (mt/user->id :lucky)]
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id originator-id}
                     :model/MetabotMessage _ {:conversation_id convo-id :user_id originator-id}
                     :model/MetabotMessage _ {:conversation_id convo-id :user_id participant-id}]
        (is (some? (mt/user-http-request :lucky :delete 403
                                         (str "metabot/conversations/" convo-id))))
        (is (t2/exists? :model/MetabotConversation :id convo-id))))))

(deftest delete-conversation-validates-profile-test
  (testing "the delete endpoint validates the requested profile"
    (let [user-id (mt/user->id :rasta)]
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id user-id}
                     :model/MetabotMessage _ {:conversation_id convo-id
                                              :user_id         user-id
                                              :profile_id      "nlq"}]
        (is (some? (mt/user-http-request :rasta :delete 404
                                         (str "metabot/conversations/" convo-id
                                              "?profile-id=internal"))))
        (is (t2/exists? :model/MetabotConversation :id convo-id))))))
      ...
      (mt/with-temp [:model/MetabotConversation {convo-id :id} {:user_id owner-id}]
        ...
        (testing "throws 403 when a non-participant tries to continue the conversation"
          (binding [api/*current-user-id* other-id]
            (is (thrown? Exception (check! convo-id)))))
