...
(deftest customization-settings-require-ai-controls-test
  (testing "without :ai-controls feature, customization settings return defaults and reject writes"
    (mt/with-premium-features #{}
      ...
      (is (= "AI机器人" (metabot.settings/metabot-name)))
...
(deftest validate-metabot-provider-accepts-openai-compatible-test
  (testing "accepts an OpenAI-compatible provider with an arbitrary model name"
    (mt/with-temporary-setting-values [llm-metabot-provider "openai-compatible/deepseek-v4-pro"]
      (is (= "openai-compatible/deepseek-v4-pro"
             (metabot.settings/llm-metabot-provider))))))
(deftest configured-provider-credentials-openai-compatible-test
  (mt/with-temporary-setting-values [llm-openai-compatible-api-key "provider-key"
                                     llm-openai-compatible-api-base-url "https://api.example/v1"]
    (is (= {:api-key "provider-key" :base-url "https://api.example/v1"}
           (metabot.settings/configured-provider-credentials "openai-compatible")))))

(deftest provider-credentials-complete-openai-compatible-test
  (is (true? (metabot.settings/provider-credentials-complete?
              "openai-compatible"
              {:api-key "provider-key" :base-url "https://api.example/v1"})))
  (is (false? (metabot.settings/provider-credentials-complete?
               "openai-compatible"
