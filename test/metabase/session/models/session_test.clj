                ...
                (mt/with-temp [:model/LoginHistory _ {:user_id   user-id
                  ...
                  (is (malli= [:sequential {:min 1}
                               [:map {:closed true}
                                [:from ms/Email]
                                [:to [:= [email]]]
                                [:subject [:= (format "数据看板新设备登录提醒：%s" first-name)]]
                                [:body [:sequential [:map
                  ...
                  (let [message  (-> @mt/inbox (get email) first :body first :content)
                    ...
                    (testing (format "\nMessage = %s\nsite-url = %s" (pr-str message) (pr-str site-url))
                      (is (string? message))
                      (when (string? message)
                        (doseq [expected-str ["检测到你的数据看板账号有一次新设备登录"
                                              (format "你好，%s。你的数据看板账号刚刚从一台新设备登录。"
                                                      first-name)
      ...
      (testing "Use first name if exists"
        (mt/with-temp [:model/User {user-id :id
                                    email :email} {:first_name "Ngoc"
                                                   :last_name  "Khuat"}]
          (is (contains? (new-login-email user-id email) "数据看板新设备登录提醒：Ngoc"))))
