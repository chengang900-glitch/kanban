...
describe("metabot reducer", () => {
  describe("conversation snapshots", () => {
    it("restores the original conversation id, messages, history, and state", () => {
      ...
      store.dispatch(
        metabotActions.setConversationSnapshot({
          ...
          messages: [{ id: "1", role: "user", type: "text", message: "hello" }],
      ...
      const conversation = store.getState().metabot.conversations.omnibot;
      expect(conversation?.conversationId).toBe(
      ...
      expect(conversation?.state).toEqual({ queries: { q1: {} } });
    });
  });
