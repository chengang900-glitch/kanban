  ...
  return (
    <MessageContainer chatRole={message.role} {...props}>
      {match(message)
        ...
        .exhaustive()}
      {!hideActions && message.type !== "reasoning" && (
        <Flex className={Styles.messageActions}>
          <Tooltip label={clipboard.copied ? t`Copied!` : t`Copy`}>
            <ActionIcon
              h="sm"
...
const AgentErroredTurnAlert = ({
  ...
  <AgentTurnAlert
    ...
    message={
      ...
      (message.error["error-code"] === "metabot_max_iterations"
        ? t`The analysis reached its step limit before a reliable final answer was ready. Retry or narrow the question.`
    ...
    () =>
      messages.filter(
        (message) =>
          ...
          (debug || isUserVisibleMessage(message)),
      ),
    [debug, messages],
  );
