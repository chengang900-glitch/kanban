...
import { OpenAICompatibleProviderFields } from "./OpenAICompatibleProviderFields";
import {
...
export const DEFAULT_HIDDEN_PROVIDERS = [
  "metabase",
  "anthropic",
  "azure",
  "bedrock",
  "openai",
  "openrouter",
] as const satisfies readonly MetabotProvider[];
...
export function AIProviderConfigurationForm({
  ...
  const connectedProvider =
    isConfigured &&
    config?.provider &&
    !hiddenProviders.includes(config.provider)
      ? config.provider
      : undefined;
  ...
  const disconnectProvider = useCallback(async () => {
    ...
    if (
      ...
      connectedProvider !== "openai-compatible"
    ) {
  ...
  const providerOptions = useMemo(() => {
    const options = Object.values(
      getProviderOptions(offerMetabaseAiManaged),
    ).filter((option) => !hiddenProviders.includes(option.value));
    return options.map((option) => ({
      ...option,
      disabled: !isAvailableProvider(option.value),
    }));
  }, [hiddenProviders, offerMetabaseAiManaged]);
