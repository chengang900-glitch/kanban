...
describe("NewItemMenu", () => {
  ...
  it("shows AI问数 when NLQ access exists but AI is not configured", async () => {
    await setup({ isConfigured: false });

    expect(await screen.findByText("AI问数")).toBeInTheDocument();
  });
  ...
  it("should support keyboard navigation", async () => {
    ...
    expect(
      await screen.findByRole("menuitem", { name: /AI问数/ }),
    ).toHaveFocus();

    await userEvent.keyboard("{ArrowDown}");

    expect(
      await screen.findByRole("menuitem", { name: /Chart/ }),
    ).toHaveFocus();
