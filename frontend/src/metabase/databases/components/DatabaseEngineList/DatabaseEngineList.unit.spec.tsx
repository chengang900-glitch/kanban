...
const setup = (hasSampleDatabase = false) => {
  const state = createMockState({
    settings: createMockSettingsState({
      engines: mockEngines,
      "has-sample-database?": hasSampleDatabase,
    }),
  });
...
describe("DatabaseEngineList", () => {
  it("should not show a provider label for the sample database", () => {
    setup(true);

    expect(screen.getByText("Sample Database for testing")).toBeInTheDocument();
    expect(screen.queryByText("(by Metabase)")).not.toBeInTheDocument();
  });
