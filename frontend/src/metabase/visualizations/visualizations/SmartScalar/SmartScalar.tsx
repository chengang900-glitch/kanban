...
export type SmartScalarAccent = "error" | "success" | undefined;

export function getSmartScalarAccent(
  ...
  if (comparisons.some(({ changeColor }) => changeColor === errorColor)) {
    return "error";
  }

  if (comparisons.some(({ changeColor }) => changeColor === successColor)) {
    return "success";
  }
...
export function SmartScalar({
  ...
  const accent = getSmartScalarAccent(comparisons, getColor);
  ...
  return (
    <ScalarWrapper>
      ...
      >
        <span
          data-smartscalar-accent={accent}
          onClick={handleClick}
          ref={scalarRef}
        >
          <ScalarValue
