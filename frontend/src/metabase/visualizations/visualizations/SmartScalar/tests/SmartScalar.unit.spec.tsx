...
import { getSmartScalarAccent } from "../SmartScalar";
...
describe("SmartScalar", () => {
  describe("dashboard accent", () => {
    const getColor = (name: string) => `color-${name}`;
    ...
    it("prefers error over success", () => {
      ...
      expect(getSmartScalarAccent(comparisons, getColor)).toBe("error");
    });

    it("uses success when there is no error and otherwise falls back to brand", () => {
      expect(
        getSmartScalarAccent(
          [{ changeColor: "color-success" }] as ComparisonResult[],
          getColor,
        ),
      ).toBe("success");
      expect(
        getSmartScalarAccent(
          [{ changeColor: "color-accent1" }] as ComparisonResult[],
          getColor,
        ),
      ).toBeUndefined();
    });
