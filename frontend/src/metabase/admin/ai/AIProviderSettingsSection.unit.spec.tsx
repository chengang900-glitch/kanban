...
const DEFAULT_RESPONSES: Record<MetabotProvider, MetabotSettingsResponse> = {
  ...
  "openai-compatible": {
    value: "openai-compatible/deepseek-v4-pro",
    models: [
...
async function setup({
  ...
  const mergedApiKeyValues: Record<
    MetabotApiKeyProvider | "azure" | "bedrock" | "openai-compatible",
    string | null
  > = {
    anthropic: "**********45",
    azure: null,
  ...
  > = {
    ...
    "llm-openai-compatible-api-base-url": createMockSettingDefinition({
      ...
      value: mergedApiKeyValues["openai-compatible"]
        ? "https://api.deepseek.com"
        : undefined,
    }),
    "llm-openai-compatible-api-protocol": createMockSettingDefinition({
      key: "llm-openai-compatible-api-protocol",
  ...
  fetchMock.post("path:/api/metabot/settings/validate", (call) => {
    ...
    return responseMap[body.provider];
  });
  ...
  const view = renderAsModal
    ? renderWithProviders(
        <AIProviderConfigurationForm
          isModal
          onClose={onClose}
          hiddenProviders={hideRestrictedProviders ? undefined : []}
        />,
        {
          storeInitialState,
        },
...
describe("AIProviderSettingsSection", () => {
  ...
  it.each([
    { renderAsModal: false, page: "admin settings" },
    { renderAsModal: true, page: "user configuration modal" },
  ])("shows only OpenAI Compatible on the $page", async ({ renderAsModal }) => {
    await setup({
      savedProviderValue: null,
      isConfigured: false,
      hideRestrictedProviders: true,
  ...
  it("validates OpenAI Compatible without saving settings", async () => {
    await setup({ savedProviderValue: null, isConfigured: false });

    await selectProvider("OpenAI Compatible");
    expect(screen.getByLabelText("API protocol")).toHaveValue(
      "Chat Completions",
    );
    await userEvent.click(screen.getByLabelText("API protocol"));
    await userEvent.click(
      await screen.findByRole("option", { name: "Responses API" }),
    );
    await userEvent.type(
      screen.getByLabelText("API base URL"),
      "https://api.deepseek.com",
    );
    await userEvent.type(screen.getByLabelText("API key"), "provider-key");
    await userEvent.type(
      screen.getByPlaceholderText("Enter a model name"),
      "deepseek-v4-pro",
    );
    await userEvent.click(
    ...
    await waitFor(() => {
      expect(
        fetchMock.callHistory.called("path:/api/metabot/settings/validate"),
      ).toBe(true);
    });
    expect(
    ...
    const validationRequest = fetchMock.callHistory.lastCall(
      "path:/api/metabot/settings/validate",
    );
    expect(validationRequest?.options?.body).toContain(
      '"protocol":"responses"',
    );
  });

  it("shows an OpenAI Compatible validation error", async () => {
    const message = "The configured model is not compatible";
    await setup({
      savedProviderValue: null,
      isConfigured: false,
      validationResponse: { status: 400, body: { message } },
    });
  ...
  it("shows pricing details in a tooltip for the Metabase provider", async () => {
    ...
    await selectProvider("Dashboard");
  ...
  it("disconnects the Metabase-managed provider by removing the add-on before clearing the provider setting", async () => {
    ...
    await screen.findByText("Connected to Dashboard");
    await confirmDisconnectProvider();
