...
describe("AISettingsPage", () => {
  ...
  it("shows MCP descriptions without docs links", async () => {
    await setup({ page: "mcp" });

    expect(
      screen.getByText("Enable external access to the Agent API."),
    ).toBeInTheDocument();
    expect(
  ...
  it("hides the embedded upsell tab when embedding is unavailable", async () => {
    await setup({
      enableEmbedding: false,
      initialRoute: `/admin/metabot?metabot_id=${FIXED_METABOT_IDS.EMBEDDED}`,
    });
    ...
    expect(
      screen.queryByRole("tab", { name: "Embedded" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: "Internal", selected: true }),
    ).toBeInTheDocument();
  });
