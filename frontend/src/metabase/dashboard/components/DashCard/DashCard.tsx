...
function DashCardInner({
  ...
  const isSdkEmbedding = isEmbeddingSdk();
  const isMainAppCard = !isGuestEmbed && !isSdkEmbedding;
  const hasTransparentBorder =
    !isEditing && isQuestionDashCard(dashcard) && !isMainAppCard;
  ...
  return (
    <ErrorBoundary>
      <Box
        ...
        data-card-display={mainCard.display}
        className={cx(
          S.DashboardCardRoot,
          S.DashCardRoot,
          DashboardS.Card,
          ...
          CS.hoverVisibility,
          {
            ...
            [S.isMainAppCard]: isMainAppCard,
            [S.isEmbeddingSdk]: isSdkEmbedding,
          },
          className,
        )}
        style={(theme) => {
