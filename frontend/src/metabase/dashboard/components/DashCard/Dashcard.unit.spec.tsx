...
function setup({
  ...
  Pick<
    ...
    "dashcardMenu" | "isEditing" | "isGuestEmbed"
  > & {
...
describe("DashCard", () => {
  ...
  it("marks main-app cards with their visualization type", () => {
    setup();

    expect(screen.getByTestId("dashcard")).toHaveAttribute(
      "data-card-display",
      "table",
    );
    expect(screen.getByTestId("dashcard")).not.toHaveStyle(
      "border: 1px solid transparent",
    );
  });

  it("keeps a guest-embed question card border transparent", () => {
    setup({ isGuestEmbed: true });

    expect(screen.getByTestId("dashcard")).toHaveStyle(
      "border: 1px solid transparent",
    );
  });

  it("does not force a transparent border while editing an embed", () => {
    setup({ isGuestEmbed: true, isEditing: true });
