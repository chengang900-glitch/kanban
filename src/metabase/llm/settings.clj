...
(defsetting llm-openai-compatible-api-key
  (deferred-tru "The API key for the OpenAI-compatible provider.")
  :sensitive?  true
  :visibility  :settings-manager
  :export?     false
  :doc         false
  :setter      (partial set-trimmed-string! :llm-openai-compatible-api-key))

(defsetting llm-openai-compatible-api-base-url
  (deferred-tru "The API base URL for the OpenAI-compatible provider.")
  :encryption  :no
  :visibility  :settings-manager
  :export?     false
  :doc         false
...
(def ^:private openai-compatible-api-protocols
  #{"chat-completions" "responses"})

(defn- set-openai-compatible-api-protocol!
  [new-value]
  (let [protocol (trimmed-string new-value)]
    (when (and protocol
               (not (contains? openai-compatible-api-protocols protocol)))
      (throw (ex-info (tru "Unsupported OpenAI-compatible API protocol {0}." (pr-str protocol))
                      {:status-code 400})))
    (setting/set-value-of-type! :string :llm-openai-compatible-api-protocol protocol)))
...
(defsetting llm-openai-compatible-api-protocol
  ...
  :export?     false
  :doc         false
  :setter      set-openai-compatible-api-protocol!)
