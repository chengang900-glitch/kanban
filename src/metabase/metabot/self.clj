...
(defn- resolve-model-lister [provider]
  ...
  (case provider
    ...
    "openai-compatible" openai-compatible/list-models
...
(defn validate-model
  "Validate provider-specific model compatibility when a provider requires it."
  [provider opts]
  (case provider
    "openai-compatible" (openai-compatible/validate-model opts)
    true))
