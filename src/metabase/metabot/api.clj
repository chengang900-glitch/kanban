  ...
  (let [enriched-context (metabot.context/create-context context {:metabot-id metabot-id
    ...
    (sr/streaming-response {:content-type "text/event-stream"} [^OutputStream os canceled-chan]
      (let [parts-atom (atom [])
            ...
            thrown     (volatile! nil)
            ;; Provider adapters may emit raw reasoning_content for DeepSeek/Qwen
            ;; compatibility. It is not part of the user-facing stream or stored history.
            xf         (comp (remove #(= :reasoning (:type %)))
                             (u/tee-xf parts-atom)
...
(defn- effective-openai-compatible-credentials
  [{:keys [api-key base-url protocol] :as supplied-credentials}]
  (cond-> {:api-key  (or (non-blank-string api-key)
                         (non-blank-string (llm.settings/llm-openai-compatible-api-key)))
           :base-url (or (llm.settings/normalize-llm-base-url base-url)
                         (llm.settings/normalize-llm-base-url
                          (llm.settings/llm-openai-compatible-api-base-url)))}
    (contains? supplied-credentials :protocol)
...
(defn- request-credentials
  ...
  (case provider
    ...
    (when (contains? body :credentials)
      (if (nil? credentials)
        {:api-key nil :base-url nil :protocol nil}
        (let [creds (effective-openai-compatible-credentials credentials)]
          (when-not (metabot.settings/provider-credentials-complete? provider creds)
            (throw (ex-info (tru "OpenAI-compatible credentials are incomplete.")
                            {:status-code  400
                             :api-error    true
                             :missing-keys (vec (remove #(non-blank-string (get creds %))
                                                        [:api-key :base-url]))})))
...
(defn- save-openai-compatible-credentials!
  [{:keys [api-key base-url] :as credentials}]
  (setting/set! :llm-openai-compatible-api-key api-key)
  (setting/set! :llm-openai-compatible-api-base-url base-url)
  (when (contains? credentials :protocol)
    (setting/set! :llm-openai-compatible-api-protocol (:protocol credentials))))
...
(defn- save-credentials!
  ...
  (when credentials
    (case provider
      ...
      "openai-compatible" (save-openai-compatible-credentials! credentials)
...
(defn- credential-setting-keys
  ...
  (case provider
    ...
    "openai-compatible" (cond-> [:llm-openai-compatible-api-key :llm-openai-compatible-api-base-url]
                          (contains? credentials :protocol)
                          (conj :llm-openai-compatible-api-protocol))
    [(provider-api-key-setting-key provider)]))

(defn- validate-openai-compatible-model!
  [provider model credentials]
  (when (= provider "openai-compatible")
    (when-not model
      (throw (ex-info (tru "A model name is required to connect an OpenAI-compatible provider.")
                      {:status-code 400
                       :api-error   true
                       :provider    provider})))
    (metabot.self/validate-model provider {:model model :credentials credentials})))

(defn- compatible-settings-response
  [provider opts]
  (let [response (settings-response provider opts)]
    (if (and (= provider "openai-compatible")
             (:credentials-error response))
      (-> response
          (dissoc :credentials-error)
          (assoc :models []))
      (throw-credentials-error! response))))

(api.macros/defendpoint :post "/settings/validate"
  :- metabot-settings-response-schema
  "Validate candidate provider credentials and return available models without saving settings."
  [_route-params
   _query-params
  ...
  (let [{:keys [provider] request-model :model} body
        ...
        model       (non-blank-string request-model)
        _           (when (and (= provider "openai-compatible") model)
                      (validate-openai-compatible-model! provider model credentials))
        response    (settings-response provider {:credentials credentials :model model})]
    (if (and (= provider "openai-compatible")
             (:credentials-error response)
             model)
      (do
        (validate-openai-compatible-model! provider model credentials)
        {:value (metabot.settings/llm-metabot-provider) :models []})
      (throw-credentials-error! response))))
...
(api.macros/defendpoint :put "/settings"
  ...
  (let [{:keys [provider] request-model :model} body
        credentials       (request-credentials provider body)
        clearing-credentials? (and (contains? body :credentials)
                                   (nil? (:credentials body)))
        ...
        model             (cond
                            ...
                            :else
                            nil)
        _                 (when (and (= provider "openai-compatible")
                                     provider-changed?
                                     (nil? model))
                            (throw (ex-info (tru "A model name is required to connect an OpenAI-compatible provider.")
                                            {:status-code 400
        ...
        _                 (when (and (= provider "openai-compatible")
                            ...
                            (validate-openai-compatible-model!
                             provider
                             (or model
                                 (when-not provider-changed?
                                   (provider-util/provider-and-model->model
                                    (metabot.settings/llm-metabot-provider))))
                             (or credentials
                                 (metabot.settings/configured-provider-credentials provider))))
        ;; The model listing validates the request credentials before anything is saved.
        response          (compatible-settings-response provider {:credentials credentials
