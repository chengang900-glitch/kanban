...
(def ^:private direct-providers
  ...
  #{"anthropic" "azure" "bedrock" "openai" "openai-compatible" "openrouter"})
...
(defn configured-provider-credentials
  ...
  (case provider
    ...
    "openai-compatible"
    (let [api-key  (non-blank (llm.settings/llm-openai-compatible-api-key))
          base-url (non-blank (llm.settings/llm-openai-compatible-api-base-url))]
      (when (and api-key base-url)
        {:api-key api-key :base-url base-url}))
...
(defn provider-credentials-complete?
  ...
  (boolean
   (case provider
     ...
     "openai-compatible" (and (non-blank (:api-key credentials))
                              (non-blank (:base-url credentials)))
