...
(def ^:private ConversationDetail
  [:map
   ...
   [:history         [:sequential :map]]
   [:state           :map]])

(def ^:private ConversationIdParams
  [:map [:id ms/UUIDString]])

(def ^:private ConversationProfileQueryParams
  [:map
   [:profile-id {:optional true} [:maybe ms/NonBlankString]]])
...
(defn- visible-conversations-clause
  [user-id profile-id]
  (cond-> [:and (participation-clause user-id)]
    profile-id (conj (profile-clause profile-id))))
...
(api.macros/defendpoint :get "/" :- ListConversationsResponse
  ...
  fall back to the conversation originator. When `profile-id` is supplied,
  only conversations containing messages for that Metabot profile are returned."
  [_route-params
   {:keys [profile-id]} :- ConversationProfileQueryParams]
  (let [user-id         (api/check-404 api/*current-user-id*)
        limit           (or (request/limit) default-limit)
        offset          (or (request/offset) default-offset)
        visible-to-user (visible-conversations-clause user-id profile-id)
...
(api.macros/defendpoint :get "/:id" :- ConversationDetail
  ...
  Accessible to any participant in the conversation or to any superuser. When
  `profile-id` is supplied, the conversation must contain that profile."
  [{:keys [id]} :- ConversationIdParams
   {:keys [profile-id]} :- ConversationProfileQueryParams]
  (api/read-check :model/MetabotConversation id)
  (when profile-id
    (api/check-404
     (t2/exists? :model/MetabotMessage :conversation_id id :profile_id profile-id)))
  (metabot.persistence/conversation-detail id))

(api.macros/defendpoint :delete "/:id"
  "Permanently delete a conversation and its messages.

  Only the conversation originator can delete it. When `profile-id` is
  supplied, the conversation must contain that profile."
  [{:keys [id]} :- ConversationIdParams
   {:keys [profile-id]} :- ConversationProfileQueryParams]
  (let [conversation (api/check-404
                      (t2/select-one [:model/MetabotConversation :id :user_id] :id id))]
    (api/check-403 (= (:user_id conversation) api/*current-user-id*))
    (when profile-id
      (api/check-404
       (t2/exists? :model/MetabotMessage :conversation_id id :profile_id profile-id)))
    (t2/delete! :model/MetabotConversation :id id)
