...
(api.macros/defendpoint :post "/:id/prompt-suggestions/regenerate"
  ...
  "Remove any existing prompt suggestions for the Metabot with `id` and generate new ones.
   ...
   `:no-library-content` when the selected collection has no supported cards or dashboards, or
...
(api.macros/defendpoint :get "/:id/prompt-suggestions"
  ...
  [{:keys [id]} :- [:map [:id pos-int?]]
   {:keys [sample model model_id]} :- [:map
                                       ...
                                       [:model {:optional true} [:enum "metric" "model" "question"]]
  ...
  (let [offset (when-not sample (request/offset))
        ...
        base-query (cond-> {:join  [[{:select [:id :name :type]
                                      :from   [[(metabot.tools.u/metabot-prompt-source-cards-query id)
                                                :scope]]}
                                     :card]
                                    [:and
