...
(defn- conversation-summary
  "Use the first user prompt as a compact history-list label."
  [content]
  (when-let [summary (some-> content str str/trim not-empty)]
...
(defn combine-text-parts-xf
  "Transducer that merges consecutive `:text` or `:reasoning` parts of the same
  type. Other parts pass through unchanged. Used before persistence so streaming
  deltas consolidate into one stored block."
  []
  (fn [rf]
...
(defn start-turn!
  ...
  (let [;; Originator: explicit `:user-id` wins; otherwise fall back to the
        ...
        summary                (conversation-summary (:content user-message))]
...
(defn- convert-content-block
  ...
  (let [block-type (:type block)
    ...
    (cond
      ...
      ;; Assistant reasoning: {:type "reasoning" :text "..."}
      (= "reasoning" block-type)
      (cond-> {:id (or (:id block) (str (random-uuid)))
               :role "agent"
               :type "reasoning"
               :message (:text block)}
...
(defn- content-block->history
  "Convert one persisted content block to the request-history shape accepted by
  the Metabot agent endpoint. Data parts are UI-only and intentionally omitted."
  [block]
  (let [block-type (:type block)
        block-role (:role block)
        history-string (fn [value nil-value]
                         (cond
                           (string? value) value
                           (nil? value)    nil-value
    ...
    (cond
      (= "user" block-role)
      {:role "user" :content (:content block)}

      (= "text" block-type)
      {:role "assistant" :content (:text block)}

      (and (= "assistant" block-role) (= "TEXT" (:_type block)))
      {:role "assistant" :content (:content block)}
      ...
      {:role "assistant"
       ...
       [{:id        (:id block)
         :name      (:function block)
         :arguments (history-string (:arguments block) "{}")}]}

      (= "tool-output" block-type)
      (let [result  (:result block)
            content (cond
                      (some? (:output result)) (:output result)
                      (some? result)           result
                      (some? (:error block))   (:error block)
                      :else                    "")]
        {:role         "tool"
         :tool_call_id (:id block)
         :content      (history-string content "")})

      :else nil)))

(defn message->history
  "Convert one persisted MetabotMessage row into resumable request history.
  Aborted rows receive synthetic tool results for calls whose result never
  arrived, matching the live frontend's abort handling."
  [message]
  (let [blocks       (or (:data message) [])
        history      (into [] (keep content-block->history) blocks)
        completed    (into #{} (keep :tool_call_id) history)
        unresolved   (for [{:keys [role tool_calls]} history
                           :when (= "assistant" role)
                           {:keys [id]} tool_calls
                           :when (not (contains? completed id))]
                       {:role         "tool"
                        :tool_call_id id
                        :content      "Tool execution interrupted by user"})]
    (cond-> history
      (and (false? (:finished message)) (seq unresolved))
      (into unresolved))))

(defn messages->history
  "Build resumable request history from stored rows, using the same visibility
  rules as the chat snapshot: omit active placeholders and errored turn pairs."
  [messages]
  (->> messages
       (remove placeholder-still-active?)
       drop-errored-pairs
       (into [] (mapcat message->history))))
