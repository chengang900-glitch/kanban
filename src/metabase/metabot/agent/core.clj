...
(defn- max-iterations-error-part
  "Return a typed, user-safe error when the model keeps requesting tools through
  the profile's iteration limit."
  []
  {:type  :error
   :error {:message    "The analysis reached its step limit before a reliable final answer was ready. Retry or narrow the question."
          ...
          (cond
            ...
            (let [reason       (finish-reason iteration max-iter terminal-tools parts)
                  final-result (cond-> result'
                                 (= reason :max-iterations) (rf (max-iterations-error-part)))]
              (log/info "Agent loop complete"
                        {:iterations iteration
                         :reason     reason})
              (assoc loop-state
                     :status :done
                     :result (rf final-result (final-state-part @memory-atom))))))))))
