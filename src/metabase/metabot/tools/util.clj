...
(defn- metabot-cards-query
  [metabot-id card-types include-dashboard-cards? {:keys [limit]}]
  (let [metabot (t2/select-one :model/Metabot :id metabot-id)
        ...
        collection-ids (when metabot-collection-id
                         (let [selected-collection (t2/select-one :model/Collection :id metabot-collection-id)]
                           (conj (collection/descendant-ids selected-collection) metabot-collection-id)))
        card-collection-filter (if collection-ids
                                 [:in :report_card.collection_id collection-ids]
                                 [:and true])
        dashboard-collection-filter (if collection-ids
                                      [:in :dashboard.collection_id collection-ids]
                                      [:and true])
        dashboard-card-ids {:select [:dashcard.card_id]
                            :from [[:report_dashboardcard :dashcard]]
                            :join [[:report_dashboard :dashboard]
                                   [:= :dashboard.id :dashcard.dashboard_id]]
                            :where [:and
                                    [:= :dashboard.archived false]
                                    dashboard-collection-filter
                                    [:not= :dashcard.card_id nil]]}
        scope-filter (cond-> [:and
                              card-collection-filter
                              [:in :report_card.type [:inline (mapv name card-types)]]]
                       include-dashboard-cards?
                       (#(into [:or %] [[:in :report_card.id dashboard-card-ids]])))
...
(defn metabot-metrics-and-models-query
  ...
  [metabot-id & {:as opts}]
  (metabot-cards-query metabot-id #{:metric :model} false opts))

(defn metabot-prompt-source-cards-query
  "Return cards that can seed suggested prompts: metrics, models and questions in the
  Metabot collection, plus saved cards used by dashboards in that collection."
  [metabot-id & {:as opts}]
  (metabot-cards-query metabot-id #{:metric :model :question} true opts))
...
(defn get-prompt-source-cards
  "Retrieve the readable cards that can be used to generate suggested prompts."
  [metabot-id & {:as opts}]
  (t2/select :model/Card (-> (metabot-prompt-source-cards-query metabot-id opts)
                             (update :order-by (fnil conj []) [:report_card.id]))))
