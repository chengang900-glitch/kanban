...
(def ^:private sqlserver-declare-prefix
  "SQL Server batches beginning with DECLARE cannot be embedded as the derived
  table produced by a source-card notebook query. Allow leading whitespace and
  SQL comments so the check matches the statement the driver will receive."
  #"(?is)^\s*(?:(?:--[^\r\n]*(?:\r?\n|$))|(?:/\*.*?\*/\s*))*DECLARE\b")

(defn- assert-composable-source-card!
  "Reject the known SQL Server native-batch shape that cannot be composed as a
  notebook source. This is a narrow preflight: ordinary SELECT-based native
  models and every non-SQL Server database keep their existing behavior."
  [parsed-query]
  (when-let [entity-id (first-stage-source-card-eid parsed-query)]
    (when-let [card (tools.u/get-card-by-entity-id entity-id)]
      (let [dataset-query (or (:dataset_query card) (:dataset-query card))
            query-type   (some-> (or (:type dataset-query) (get dataset-query "type")) name)
            sql          (or (get-in dataset-query [:native :query])
                             (get-in dataset-query ["native" "query"]))
            engine       (some-> (t2/select-one-fn :engine :model/Database :id (:database_id card)) name)]
        (when (and (= "sqlserver" engine)
                   (= "native" query-type)
                   (string? sql)
                   (re-find sqlserver-declare-prefix sql))
          (throw
           (ex-info
            (tru (str "The selected SQL Server model starts with `DECLARE`, so Metabase cannot "
                      "embed it as a source for filters, grouping, or charts. Use a SELECT-based "
                      "model/view, or choose another composable data source."))
            {:agent-error? true
             :status-code  400
             :error        :non-composable-native-source
             :entity-id    entity-id})))))))
...
(defn execute-representations-query
  ...
  (let [parsed      (try
        ...
        _source-ok  (assert-composable-source-card! parsed)
