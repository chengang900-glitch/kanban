    ...
    (cond->> results
      ...
      (mapv (fn [r]
              (if-let [{:keys [table-id schema table-name]}
                       (when (= "metric" (:type r))
                         (get metric-id->table-info (:id r)))]
                (cond-> (assoc r
                               :base_table_id table-id
                               :base_table_name table-name
                               :base_table_schema schema)
                  (:database_name r)
                  (assoc :base_table_portable_fk [(:database_name r) schema table-name]))
                r))))))
...
(defn- normalize-limit
  ...
  (let [parsed (cond
                 (integer? limit) limit
                 (string? limit)  (parse-long (str/trim limit))
                 :else            nil)]
    (when (and parsed (pos? parsed))
      (min max-search-limit parsed))))

(defn- normalize-entity-types
  [entity-types]
  (cond
    (nil? entity-types)        nil
    (sequential? entity-types) (vec entity-types)
    (string? entity-types)     (let [decoded (try
                                               (json/decode+kw entity-types)
...
(defn- do-search
  ...
  (let [entity-types (normalize-entity-types entity_types)
        parsed-limit (if (nil? limit) default-search-limit (normalize-limit limit))]
    (cond
      (nil? parsed-limit)
      {:output (str "Invalid limit for " label ": expected a positive integer up to " max-search-limit ".")}

      (not (or (nil? entity-types) (sequential? entity-types)))
      {:output (str "Invalid entity_types for " label ": expected an array of entity type names.")}

      :else
      (if-let [invalid (invalid-entity-types entity-types allowed-types)]
        ...
        (try
          (let [results (search (merge {:semantic-queries semantic_queries
                                        ...
                                        :entity-types    (or (seq entity-types) (vec allowed-types))
