...
(defn format-reasoning-line
  "Format reasoning part as AI SDK v4 line: g:\"reasoning content\"."
  [{:keys [text]}]
  (str "g:" (json/encode text)))
...
(defn aisdk-line-xf
  ...
  Input types and their output:
    :text       -> 0:\"content\"
    :reasoning  -> skipped (provider compatibility only; never user-visible)
...
(defn request
  ...
  (http/request (-> req
                    ...
                    (assoc :socket-timeout (llm/llm-request-timeout-ms)
